window.bioMp = function(el, options) {
  ...
  function create() {
    ...
    // Add CSS
    var wrapCss = 'width: ' + width + 'px; height: ' + height +
      'px; overflow: hidden; transform-origin: 0 0 0;';
    var phoneCss = 'position: relative; top: 0px; left: 0px; width: ' +
      selView.width +
      'px; height: ' + selView.height +
      'px; background: none; -webkit-transform-origin: 0 0 0; -moz-transform-origin: 0 0 0; -ms-transform-origin: 0 0 0; -o-transform-origin: 0 0 0; transform-origin: 0 0 0; -webkit-transform: scale(' +
      scale + '); -moz-transform: scale(' + scale +
      '); -ms-transform: scale(' +
      scale + '); -o-transform: scale(' + scale + '); transform: scale(' +
      scale +
      ');';
    var phoneImageCss =
    ...
    if (view == 'front')
      var screenCss =
        'position: absolute; top: 99px; left: 26.5px; width: 375px; height: 671px; border: 0;';
    else if (view == 'left')
      var screenCss =
...
function toggle() {
  if (navstate == "closed") {
    document.getElementById("topnav").style.height = "270px";
    navstate = "open";
  } else {
